...
def dataloader(train_data, val_data, test_data, batch_size=1):
    def load_image_pair(hazy_path, clear_path):
        ...
        # Normalization in the pre processing layer
...
def dehaze_net():
    ...
    # Extracts channel-wise features (like color weighting)
    conv1 = Conv2D(16, 1, padding='same', activation='relu')(inputs) # 16 - No of filters & 1 - 1x1 - Filter size
    # Captures local textures and edges.
    conv2 = Conv2D(16, 3, padding='same', activation='relu')(conv1)
    # Combines both low-level and texture features.
    concat1 = Concatenate()([conv1, conv2])

    # Extracts broader context from the combined features.
    conv3 = Conv2D(16, 5, padding='same', activation='relu')(concat1)
    # Merges texture + broader context.
    concat2 = Concatenate()([conv2, conv3])

    # Captures even more global patterns.
    conv4 = Conv2D(16, 7, padding='same', activation='relu')(concat2)
    # Merge all feature levels.
    concat3 = Concatenate()([conv1, conv2, conv3, conv4])

    # Blends all extracted information.
    conv5 = Conv2D(16, 3, padding='same', activation='relu')(concat3)
    # Produces K-map (a 3-channel transformation map).
    K = Conv2D(3, 1, padding='same')(conv5)  # K-map/Feature Map
...
if os.path.exists(MODEL_PATH):
    ...
    model.compile(optimizer=Adam(learning_rate=1e-3), loss=perceptual_loss + MeanSquaredError(), metrics=['mse'])
...
# Load VGG19 and select an intermediate feature layer
vgg = VGG19(include_top=False, weights='imagenet', input_shape=(720, 1280, 3))
vgg.trainable = False  # Freeze VGG weights

# Extract feature maps from a mid-level layer
feature_extractor = Model(inputs=vgg.input, outputs=vgg.get_layer('conv3').output)

def perceptual_loss(y_true, y_pred):
    y_true = tf.image.resize(y_true, (720, 1280))
    y_pred = tf.image.resize(y_pred, (720, 1280))
